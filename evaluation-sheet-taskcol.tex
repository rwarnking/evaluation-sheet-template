...
\newcommand\clipstring[2][\headerheight]{%
    \clipbox{0pt {\dimexpr\height - (#1/2)\relax}}{#2}% chktex 8
}

\newcommand\clipstringLR[2][3cm]{%
    \clipbox{{\dimexpr (\width - (#1))/2 \relax} 0pt}{#2}% chktex 8
}
...
%
% TODO: remove \else where it is not needed (other document also)
% TODO: adjust header to set height
% TODO: include general info file
% TODO: use clipsting for all cells
% TODO: adjust clipstring to cut right instead of both sides?
% Notes:
...
\newcommand{\printtitlerow}{%
    ...
    {%
        ...
        \if\IsPositive{\curtask}%
            \DTLforeach*{assignments_db}% database label
            {\tmptask=\taskidKey}{%
                % Split the assignement id at the - character
                \expandafter\DTLsplitstring\expandafter{\tmptask}{-}{\beforepart}{\afterpart}%
                % Count how often the task number is part of this assignment
                \StrCount{\beforepart}{\curtask}[\tmp]%
                % If there was exactly one occurance it is a subtask
                \ifnum\tmp=1%
            ...
            \multicolumn{\value{subtasks}}{c|}{%
                \clipstringLR[
                    (\tablecellwidth+\tabcolsep+\tabcolsep)*\value{subtasks}-\tabcolsep-\tabcolsep
                ]{\curtitle}%
            }%
        \fi%
    }%
...
\begin{document}
    ...
    {%
        ...
        \if\IsPositive{\curtask}%
            ...
            \curtask{} has \arabic{subtasks} subtasks\\
        \fi%
    }%
